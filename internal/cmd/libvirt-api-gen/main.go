...
func main() {
	...
	if err := run(apiPath, packageDir, output); err != nil {
		fmt.Fprintln(os.Stderr, "libvirt-api-gen:", err)
		os.Exit(1)
	}
}

func run(apiPath, packageDir, output string) error {
	...
	symbols, err := discoverNativeSymbols(packageDir, filepath.Clean(outputPath))
	if err != nil {
		return err
	}
	if len(symbols) == 0 {
		return errors.New("no vir* native symbol usages found in the package")
	}
...
func renderGenerated(document *apiDocument, symbols []string, sourceHash string) ([]byte, error) {
	...
	for _, symbol := range symbols {
		...
		signature, err := goFunctionSignature(function)
		if err != nil {
	...
	for _, symbol := range symbols {
		fmt.Fprintf(&output, "\t\t{name: %q, target: &api.%s},\n", symbol, symbol)
	}
	fmt.Fprintln(&output, "\t}")
	fmt.Fprintln(&output, "}")
	fmt.Fprintln(&output)
...
func goFunctionSignature(function apiFunction) (string, error) {
	args := make([]string, len(function.Args))
	for i, argument := range function.Args {
		mapped, err := goABIType(argument.Type, false)
		if err != nil {
			return "", fmt.Errorf("argument %s: %w", argument.Name, err)
		}
		args[i] = mapped
	}
	result, err := goABIType(function.Return.Type, true)
	if err != nil {
...
func goABIType(cType string, result bool) (string, error) {
	typeName := strings.Join(strings.Fields(cType), " ")
	...
	if strings.HasSuffix(typeName, "Ptr") {
		...
		// Opaque libvirt handles are pointer typedefs. Additional C pointer
		// levels are represented by the address of one pointer-sized slot.
		return "*unsafe.Pointer", nil
	}
	if pointerDepth > 0 {
		if pointerDepth > 1 {
			return "", fmt.Errorf("unsupported pointer depth %d for %q", pointerDepth, cType)
		}
		switch typeName {
